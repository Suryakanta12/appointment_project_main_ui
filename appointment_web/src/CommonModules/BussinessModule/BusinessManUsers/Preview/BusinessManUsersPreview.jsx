...
export default function BusinessManUsersPreview(props) {
  const bussinessTypeColumns = [
    ...
    { field: "Businessman_User_Id", headerName: "ID", minWidth: 100 },
    {
      field: "User_Id",
      headerName: "User Id",
      minWidth: 150,
    ...
    { field: "Business_Code", headerName: "Business Code", minWidth: 100 },
    { field: "Business_Status", headerName: "Business Status", minWidth: 100 },
    { field: "Bussiness_Logo", headerName: "Bussiness Logo", minWidth: 100 },
    {
      field: "Bussiness_Banner",
      headerName: "Bussiness Banner",
      minWidth: 100,
    },
    {
      field: "Bussiness_Description",
  ...
  const handleConfirmDelete = () => {
    ...
    const deletePromises = pendingDeleteIDs.map((id) =>
      deleteRecord(
        `api/v1/businessmanuser/delete-businessmanusers/${id}`,
      ),
    );
    Promise.all(deletePromises)
      .then((responses) => {
        ...
        if (failedDeletes.length > 0) {
          ...
          setSnackOptions({
            color: "success",
            message: "Selected business Man User deleted successfully.",
          });

          const updatedRows = allBusinessManUsers.filter(
            (row) => !pendingDeleteIDs.includes(row.Businessman_User_Id),
          );

          setAllBusinessManUsers(updatedRows);
          setSelectedIDs([]);
        }
      })
      .catch((err) => {
  ...
  const handleEditClick = (row) => {
    setThisBusinessManUsers(row); // Set the selected user type
    ...
    <React.Fragment>
      ...
      >
        {loading ? (
          ...
          <DataGrid
            rows={allBusinessManUsers}
            columns={bussinessTypeColumns}
            checkboxSelection
            disableRowSelectionOnClick
            pagination
            pageSizeOptions={[5, 10, 25]}
            getRowId={(row) => row.Businessman_User_Id}
            initialState={{
