...
export default function BusinessCategoriesPreview(props) {
  const bussinessTypeColumns = [
    ...
    { field: "Business_Category_Id", headerName: "ID", minWidth: 100 },
    {
      field: "Business_Type_Id",
      headerName: "Bussiness Type Name",
      minWidth: 150,
    },
    {
    ...
    {
      field: "Business_Category_Short_Name",
      headerName: "Business Category Short Name",
      minWidth: 150,
    },
    {
      field: "Business_Category_Code",
      headerName: "Business Category Code",
      minWidth: 100,
    },
    {
      field: "Business_Category_Description",
      headerName: "Business Category Description",
      minWidth: 100,
    },
    { field: "Is_Active", headerName: "Active", minWidth: 100 },
    { field: "Business_Category_Media", headerName: "Media", minWidth: 100 },
  ...
  const handleConfirmDelete = () => {
    setProcessing(true);
      const deletePromises = pendingDeleteIDs.map((id) =>
        deleteRecord(
          `api/v1/businesscategories/delete-businesscategories/${id}`,
        ),
      );
    Promise.all(deletePromises)
        ...
        if (failedDeletes.length > 0) {
          ...
          setSnackOptions({
            color: "success",
            message: "Selected Business Categories deleted successfully.",
          });

          const updatedRows = allBusinessCategories.filter(
            (row) => !pendingDeleteIDs.includes(row.Business_Category_Id),
            );
           
          setAllBusinessCategories(updatedRows);
          setSelectedIDs([]);
        }
      })
      .catch((err) => {
  ...
  const handleEditClick = (row) => {
    setThisBusinessCategories(row); // Set the selected user type
    ...
    <React.Fragment>
      ...
      >
        {loading ? (
          ...
          <DataGrid
            rows={allBusinessCategories}
            columns={bussinessTypeColumns}
            checkboxSelection
            disableRowSelectionOnClick
            pagination
            pageSizeOptions={[5, 10, 25]}
            getRowId={(row) => row.Business_Category_Id}
            initialState={{
